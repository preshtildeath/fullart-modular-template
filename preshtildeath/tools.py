...
def get_layer(name, *group):
    """
    Retrieve layer object.
    @param name: Name of the layer
    @param *group: Parent group names or object.
...
def get_layer_set(name, *group):
    ...
    #         # Set name given
    #         layer_set = layer_set.layerSets.getByName(item)
    #     elif item.typename == "LayerSet":
    #         # Set object given
    #         layer_set = item
    # # Look through entire document
    # if isinstance(name, str):
    #     return layer_set.layerSets.getByName(name)
    # if name.typename == "LayerSet":
    #     return name
    layer_set = app.activeDocument
    if len(group) > 0:
        layer_set = get_layer_set(*group)
...
def text_layer_bounds (layer):
    magic_wand_select(layer, 0, 0, c=False)
    app.activeDocument.selection.invert()
    return app.activeDocument.selection.bounds


def text_layer_dimensions (layer):
...
def inside_of(inner, outer):
    test = bool(
        inner.bounds[0] >= outer.bounds[0] and
        inner.bounds[1] >= outer.bounds[1] and
        inner.bounds[2] <= outer.bounds[2] and
...
def get_set_pdf(code):
    ...
    if "svg_uri" not in dir():
        svg_uri = f"https://c2.scryfall.com/file/scryfall-symbols/sets/{key.lower()}.svg"
...
def layer_mask_select (layer):
    ...
    chnl = cid("Chnl")
    ref_mask.putEnumerated(chnl, chnl, cid("Msk "))
    desc_mask.putReference(cid("null"), ref_mask)
    desc_mask.putBoolean(cid("MkVs"), True)
    app.executeAction(cid("slct"), desc_mask, 3)
    return
...
def get_layer_index (layerID):
    ...
    except: return app.executeActionGet(ref).getInteger(cid("ItmI"))
...
def layer_styles_visible(layer, visible):
    if visible: show_hide = cid("Shw ")
    else: show_hide = cid("Hd  ")
    old_layer = app.activeDocument.activeLayer
    app.activeDocument.activeLayer = layer
    desc1 = ps.ActionDescriptor()
    list1 = ps.ActionList()
